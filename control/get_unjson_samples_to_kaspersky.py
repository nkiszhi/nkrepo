...
for i in hexstring:
    for j in hexstring:
        for k in hexstring:
            for l in hexstring:
                folder_list.append("/nkrepo/DATA/sha256/"+i+"/"+j+"/"+k+"/"+l)
for i in hexstring_0:
    for j in hexstring:
        for k in hexstring:
            for l in hexstring:
                folder_list_0.append("/nkrepo/DATA/sha256/"+i+"/"+j+"/"+k+"/"+l)
for i in hexstring_4:
    for j in hexstring:
        for k in hexstring:
            for l in hexstring:
                folder_list_4.append("/nkrepo/DATA/sha256/"+i+"/"+j+"/"+k+"/"+l)
for i in hexstring_8:
    for j in hexstring:
        for k in hexstring:
            for l in hexstring:
                folder_list_8.append("/nkrepo/DATA/sha256/"+i+"/"+j+"/"+k+"/"+l)
for i in hexstring_c:
    for j in hexstring:
        for k in hexstring:
            for l in hexstring:
                folder_list_c.append("/nkrepo/DATA/sha256/"+i+"/"+j+"/"+k+"/"+l)
...
def get_unjsonfile_0(folder):
    ...
    for fil in files:
        if len(fil)==64 and not os.path.exists(folder+"/"+fil+".json") and not os.path.exists(folder+"/"+fil+".feat"):
