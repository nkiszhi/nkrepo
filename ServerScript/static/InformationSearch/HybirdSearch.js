...
nextPage.onclick = function(){
    if(resultList.children[0].innerText!="No result")  page += 1;
...
submitButton.onclick  = function(){
    ...
    // 向网站发送搜索请求，等待结果
    fetch(window.parent.location.href + "search?HybirdSearch=" + inputContent.value + "&&Page=" + page).then(function(response){response.json().then(function(searchResult){
        // 标记非搜索状态
        ...
        // 加载页数信息
        allPage.innerText = page.toString();
