...
class AVDistributedClient:
    ...
    def _scan_single_engine(
        ...
        Args:
            engine: 杀毒引擎名称
            file_paths: 待扫描文件路径列表（与 file_contents 二选一）
            timeout: 超时时间(秒)
            file_contents: 预加载的文件内容 [(name, bytes), ...]，避免并发打开文件
        ...
        try:
            # 准备文件 — 优先使用预加载的 bytes，避免并发 open() 导致的 Errno 22
            files = []
            if file_contents:
            ...
            if 'results' in result:
                ...
                for file_name, detection in result['results'].items():
                    ...
                    if isinstance(detection, dict):
                        # 新版 VM 返回 {"detected": 0|1, "label": "..."}
                        filtered_results[file_name] = detection.get("detected", -1)
                        if "label" in detection:
                            # 用特殊后缀保存 label，避免和文件名冲突
                            filtered_results[f"__label__{file_name}"] = detection["label"]
                    else:
                        # 单引擎结果，直接使用（兼容旧版返回裸数字）
    ...
    def scan_files(
        ...
        Args:
            ...
            max_workers: 最大并发数
            timeout: 单个引擎超时时间(秒)
            file_contents: 预加载的文件内容 [(name, bytes), ...]
        ...
        # 预加载所有文件到内存，避免 15 个线程同时 open() 导致 Errno 22
        if file_contents:
            file_contents_list = file_contents
            file_paths = [c[0] for c in file_contents]  # 用于后续查找
        else:
            file_contents_list = []
            for fp in file_paths:
                with open(fp, 'rb') as f:
                    file_contents_list.append((Path(fp).name, f.read()))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_engine = {
                executor.submit(
                    self._scan_single_engine, engine,
                    file_contents=file_contents_list, timeout=timeout
                ): engine
        ...
        for result in results:
            if result.get('success') and 'results' in result:
                ...
                for file_name, value in result['results'].items():
                    if file_name in file_results:
                        # 存检测值（兼容新旧格式）
                        if isinstance(value, dict):
                            file_results[file_name]['engines'][engine] = value.get("detected", -1)
                        else:
                            file_results[file_name]['engines'][engine] = value
                        file_results[file_name]['total_engines'] += 1
                        # 判断是否检出
                        if value == 1 or (isinstance(value, dict) and value.get("detected") == 1):
                            file_results[file_name]['detection_count'] += 1
                        # 提取 label
                        label_key = f"__label__{file_name}"
                        if label_key in result['results']:
                            if 'labels' not in file_results[file_name]:
                                file_results[file_name]['labels'] = {}
                            file_results[file_name]['labels'][engine] = result['results'][label_key]
    ...
    def scan_single_file(
        ...
        Args:
            file_path: 待扫描文件路径（与 file_content 二选一）
            engines: 要使用的引擎列表
            max_workers: 最大并发数
            timeout: 超时时间
            file_content: 预加载的文件内容（bytes）
            file_name: file_content 对应的文件名
        ...
        if file_content is not None:
            return self.scan_files(
                file_contents=[(file_name or "unknown.bin", file_content)],
                engines=engines, max_workers=max_workers, timeout=timeout,
            )
    ...
    def scan_single_file_streaming(
        ...
        Args:
            ...
            max_workers: 最大并发数
            timeout: 超时时间
            file_content: 预加载的文件内容（bytes），避免并发打开文件
            file_name: file_content 对应的文件名
        ...
        # 确定文件名和文件内容
        if file_content is not None:
            f_name = file_name or "unknown.bin"
            f_content = file_content
        elif file_path:
            f_name = Path(file_path).name
            f_content = open(file_path, 'rb').read()
        else:
        ...
        # 预加载文件内容到内存，传给每个线程，避免 15 个线程同时 open() 导致 Errno 22
        file_contents_list = [(f_name, f_content)]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_engine):
                ...
                try:
                    ...
                    # 提取单个文件的结果
                    if result.get('success') and 'results' in result:
                        detection = result['results'].get(f_name, -1)

                        # 确定状态（兼容新旧格式）
                        if isinstance(detection, dict):
                            detected_val = detection.get("detected", -1)
                            label = detection.get("label", "")
                        else:
                            detected_val = detection
                            label = ""

                        # 也检查 __label__ 前缀
                        label_key = f"__label__{f_name}"
                        if label_key in result['results']:
                            label = result['results'][label_key]
...
def print_scan_results(result: Dict[str, Any]):
    ...
    for file_name, file_result in result['file_results'].items():
        ...
        for engine, detection in file_result['engines'].items():
            label = file_result.get('labels', {}).get(engine, '')
            label_str = f" [{label}]" if label else ""
            status = "🔴 恶意" if detection == 1 else "🟢 正常"
            print(f"     {engine}: {status}{label_str}")
