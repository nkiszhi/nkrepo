...
export default {  
  ...
  methods: {  
    searchVirus() {  
      ...
      axios.post('http://10.134.2.27:5000/query_platform', { tableName: this.tableName })  
        .then(response => {  
    ...
    toggleDetails(sha256) {  
     ...
     axios.get(`http://10.134.2.27:5000/detail_platform/${sha256}`) 
         .then(response => {  
    ...
    downloadFile(sha256) {  
      // 构造下载链接  
      const downloadUrl = `http://10.134.2.27:5000/download_platform/${sha256}`;  
