...
def _sanitize_mysql_identifier(identifier: str) -> str:
    """校验MySQL标识符，仅允许字母、数字和下划线。"""
    if not isinstance(identifier, str):
        raise ValueError("MySQL标识符必须是字符串")

    identifier = identifier.strip()
    if not MYSQL_IDENTIFIER_RE.fullmatch(identifier):
...
def _quote_mysql_identifier(identifier: str) -> str:
    """安全引用已校验的MySQL标识符。"""
    return f"`{_sanitize_mysql_identifier(identifier)}`"


def _resolve_query_table(cursor, table_prefix: str, short_table_name: str) -> str:
    """将前端短表名解析为当前数据库中真实存在的表名。"""
    prefix = _sanitize_mysql_identifier(table_prefix)
    suffix = _sanitize_mysql_identifier(short_table_name)
    table_name = _sanitize_mysql_identifier(f"{prefix}_{suffix}")
    ...
    return _sanitize_mysql_identifier(resolved_table_name)
...
async def query_family(request: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # 查询数据库 - 使用family数据库
        sha256s = custom_mysql_query(short_table_name, db_family, 'family')
        
        if not sha256s:
            logger.warning(f"未找到family相关数据: table={short_table_name}")
            return {'error': '未找到相关数据'}
        
        logger.info(f"家族查询成功: table={short_table_name}, 结果数量={len(sha256s)}")
...
def custom_mysql_query(short_table_name: str, db_name: str, table_prefix: str, limit: int = 20):
    """
    ...
    try:
        ...
        table_name = _resolve_query_table(cursor, table_prefix, short_table_name)
        quoted_table_name = _quote_mysql_identifier(table_name)
        
        # 执行查询
        query = f"SELECT sha256 FROM {quoted_table_name} ORDER BY RAND() LIMIT %s"
        ...
        logger.error(f"自定义查询失败：db={db_name}, table={short_table_name}, error={str(e)}")
        return None
    finally:
        if cursor:
            cursor.close()
...
async def query_platform(request: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # 查询数据库 - 使用platform数据库
        sha256s = custom_mysql_query(short_table_name, db_platform, 'platform')
        
        if not sha256s:
            logger.warning(f"未找到platform相关数据: table={short_table_name}")
            return {'error': '未找到相关数据'}
        
        logger.info(f"平台查询成功: table={short_table_name}, 结果数量={len(sha256s)}")
        return {'sha256s': sha256s}
...
async def query_category(request: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # 查询数据库 - 使用category数据库
        sha256s = custom_mysql_query(short_table_name, db_category, 'category')
        
        if not sha256s:
            logger.warning(f"未找到category相关数据: table={short_table_name}")
            return {'error': '未找到相关数据'}
        
        logger.info(f"分类查询成功: table={short_table_name}, 结果数量={len(sha256s)}")
        return {'sha256s': sha256s}
