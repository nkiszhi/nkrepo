"""
Dolos代码同源性检测服务层 - Docker版本
适用于无法直接安装Dolos命令行工具的环境
"""
import asyncio
from pathlib import Path
from typing import List, Dict, Any, Optional
import json
from datetime import datetime
import uuid
import logging
import subprocess

logger = logging.getLogger(__name__)


class DolosAnalyzer:
    """Dolos分析器 - Docker版本"""
    
    def __init__(self):
        self.results_dir = Path("dolos_results")
        self.results_dir.mkdir(exist_ok=True)
        self.temp_dir = Path("uploads/dolos_temp")
        self.temp_dir.mkdir(parents=True, exist_ok=True)
        
        # 检查是否使用Docker
        self.use_docker = self._check_docker_available()
        
    def _check_docker_available(self) -> bool:
        """检查Docker是否可用"""
        try:
            result = subprocess.run(
                ["docker", "--version"],
                capture_output=True,
                text=True
            )
            if result.returncode == 0:
                logger.info("检测到Docker，将使用Docker运行Dolos")
                return True
        except Exception:
            pass
        
        # 检查dolos命令是否可用
        try:
            result = subprocess.run(
                ["dolos", "--version"],
                capture_output=True,
                text=True
            )
            if result.returncode == 0:
                logger.info("检测到Dolos命令行工具")
                return False
        except Exception:
            pass
        
        logger.warning("未检测到Dolos或Docker，请安装其中之一")
        return False
    
    async def analyze(self, file_paths: List[str]) -> Dict[str, Any]:
        """
        执行Dolos分析
        
        Args:
            file_paths: 文件路径列表
        
        Returns:
            分析结果字典
        """
        analysis_id = str(uuid.uuid4())
        timestamp = datetime.now().isoformat()
        
        # 输出文件路径
        output_file = self.results_dir / f"{analysis_id}.json"
        
        if self.use_docker:
            # 使用Docker运行
            cmd = self._build_docker_command(file_paths, output_file)
        else:
            # 使用命令行运行
            cmd = self._build_cli_command(file_paths, output_file)
        
        logger.info(f"执行Dolos分析: {' '.join(cmd)}")
        
        # 异步执行命令
        process = await asyncio.create_subprocess_exec(
            *cmd,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        
        stdout, stderr = await process.communicate()
        
        if process.returncode != 0:
            error_msg = stderr.decode() if stderr else "未知错误"
            logger.error(f"Dolos分析失败: {error_msg}")
            raise Exception(f"Dolos分析失败: {error_msg}")
        
        # 读取结果
        try:
            with open(output_file, 'r', encoding='utf-8') as f:
                result = json.load(f)
        except Exception as e:
            logger.error(f"读取Dolos结果失败: {str(e)}")
            raise Exception(f"读取分析结果失败: {str(e)}")
        
        # 添加元数据
        result['analysis_id'] = analysis_id
        result['timestamp'] = timestamp
        result['files'] = [Path(p).name for p in file_paths]
        
        # 处理结果格式
        if 'pairs' not in result:
            result['pairs'] = self._parse_dolos_result(result, file_paths)
        
        # 保存完整结果
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(result, f, indent=2, ensure_ascii=False)
        
        logger.info(f"Dolos分析完成: {analysis_id}")
        return result
    
    def _build_docker_command(self, file_paths: List[str], output_file: Path) -> List[str]:
        """构建Docker命令"""
        # 获取文件所在目录
        file_dir = Path(file_paths[0]).parent.absolute()
        
        # 构建Docker命令
        cmd = [
            "docker", "run", "--rm",
            "-v", f"{file_dir}:/data",
            "-v", f"{self.results_dir.absolute()}:/output",
            "ghcr.io/dodona-edu/dolos:latest",
            "dolos", "run",
            "-f", "json",
            "-o", f"/output/{output_file.name}",
        ]
        
        # 添加文件路径（相对于挂载点）
        for file_path in file_paths:
            file_name = Path(file_path).name
            cmd.append(f"/data/{file_name}")
        
        return cmd
    
    def _build_cli_command(self, file_paths: List[str], output_file: Path) -> List[str]:
        """构建命令行命令"""
        cmd = [
            "dolos",
            "run",
            "-f", "json",
            "-o", str(output_file),
            *file_paths
        ]
        return cmd
    
    def _parse_dolos_result(self, result: Dict, file_paths: List[str]) -> List[Dict]:
        """
        解析Dolos结果格式
        
        Args:
            result: 原始结果
            file_paths: 文件路径列表
        
        Returns:
            标准化的结果列表
        """
        pairs = []
        
        # Dolos可能返回不同的格式，这里做兼容处理
        if 'fragments' in result:
            # 按文件对分组
            file_pairs = {}
            for fragment in result['fragments']:
                left_file = fragment.get('leftFile', '')
                right_file = fragment.get('rightFile', '')
                pair_key = f"{left_file}:{right_file}"
                
                if pair_key not in file_pairs:
                    file_pairs[pair_key] = {
                        'leftFile': left_file,
                        'rightFile': right_file,
                        'fragments': [],
                        'similarity': 0,
                        'overlap': 0
                    }
                
                file_pairs[pair_key]['fragments'].append(fragment)
            
            # 计算相似度
            for pair_key, pair_data in file_pairs.items():
                pair_data['overlap'] = len(pair_data['fragments'])
                # 这里可以根据片段数和文件大小计算相似度
                # 简化处理：假设每个片段贡献一定相似度
                pair_data['similarity'] = min(1.0, pair_data['overlap'] * 0.1)
                pairs.append(pair_data)
        
        return pairs
    
    async def get_history(
        self,
        skip: int = 0,
        limit: int = 10,
        search: Optional[str] = None,
        start_date: Optional[str] = None,
        end_date: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        获取分析历史
        
        Args:
            skip: 跳过记录数
            limit: 返回记录数
            search: 搜索关键词
            start_date: 开始日期
            end_date: 结束日期
        
        Returns:
            历史记录字典
        """
        results = []
        all_files = sorted(self.results_dir.glob("*.json"), key=lambda x: x.stat().st_mtime, reverse=True)
        
        for file in all_files:
            try:
                with open(file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                
                # 搜索过滤
                if search:
                    search_lower = search.lower()
                    if (search_lower not in data.get('analysis_id', '').lower() and
                        not any(search_lower in str(f).lower() for f in data.get('files', []))):
                        continue
                
                # 日期过滤
                timestamp = data.get('timestamp', '')
                if start_date and timestamp < start_date:
                    continue
                if end_date and timestamp > end_date + ' 23:59:59':
                    continue
                
                results.append({
                    'analysis_id': data.get('analysis_id'),
                    'timestamp': data.get('timestamp'),
                    'files': data.get('files', []),
                    'file_count': len(data.get('files', [])),
                    'pair_count': len(data.get('pairs', []))
                })
            except Exception as e:
                logger.error(f"读取历史记录失败 {file}: {str(e)}")
                continue
        
        total = len(results)
        items = results[skip:skip+limit]
        
        return {
            'items': items,
            'total': total,
            'skip': skip,
            'limit': limit
        }
    
    async def get_result(self, analysis_id: str) -> Optional[Dict]:
        """
        获取特定分析结果
        
        Args:
            analysis_id: 分析ID
        
        Returns:
            分析结果字典
        """
        result_file = self.results_dir / f"{analysis_id}.json"
        if not result_file.exists():
            return None
        
        try:
            with open(result_file, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception as e:
            logger.error(f"读取分析结果失败: {str(e)}")
            return None
    
    async def delete_result(self, analysis_id: str) -> bool:
        """
        删除分析结果
        
        Args:
            analysis_id: 分析ID
        
        Returns:
            是否删除成功
        """
        result_file = self.results_dir / f"{analysis_id}.json"
        if not result_file.exists():
            return False
        
        try:
            result_file.unlink()
            logger.info(f"删除分析结果: {analysis_id}")
            return True
        except Exception as e:
            logger.error(f"删除分析结果失败: {str(e)}")
            return False
