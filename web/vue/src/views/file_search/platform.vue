<template>  
  <main>
    ...
    <div class="input-container"> 
      <input type="text" class="input-text" placeholder="请输入一个病毒平台进行查询" v-model="tableName" name="platform" />  
      <button type="button" class="btn btn-outline-primary" @click="searchVirus" name="detect-platform">  
        <!-- 嵌入搜索图标 -->  
...
import pieChart from '../dashboard/sample/admin/components/PieChart';  
export default {  
  data() {  
    return {  
      tableName: '',  
      ...
      isLoading: false,  
      searchResults: [], 
      apiBaseUrl: 'http://xxxx42:5005' // 默认API地址
    };  
  },
  ...
  methods: {  
    ...
    searchVirus() {  
      ...
      // 使用动态API地址
      axios.post(`${this.apiBaseUrl}/query_platform`, { tableName: this.tableName })  
        .then(response => {  
    ...
    toggleDetails(sha256) {  
     ...
     // 使用动态API地址
     axios.get(`${this.apiBaseUrl}/detail_platform/${sha256}`) 
         .then(response => {  
    ...
    downloadFile(sha256) {  
      // 构造动态下载链接
      const downloadUrl = `${this.apiBaseUrl}/download_platform/${sha256}`;  
