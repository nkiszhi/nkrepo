<template>
  <main>
    ...
    <div class="input-container">
      <input v-model="tableName" type="text" class="input-text" placeholder="请输入一个病毒家族进行查询" name="family">
      <button type="button" class="btn btn-outline-primary" name="detect-family" :disabled="isSearchElLoading" @click="searchVirus">
      ...
      <ul class="flex-container">
        <li v-for="(sha256, index) in searchResults" :key="sha256" class="table-row" @mouseover="hoveringOver = sha256" @mouseleave="hoveringOver = null" @click="toggleDetails(sha256)">
          ...
          <div v-if="showDetails[sha256]" class="table-cell details-cell" style="width:90%; margin: 0 auto;">
            ...
            <p>SHA256: {{ details[sha256]['SHA256'] || sha256 }}</p>
  ...
  methods: {
    ...
    searchVirus() {
      if (!this.tableName.trim()) {
        this.$message?.warning('请输入病毒家族后再查询') || alert('请输入病毒家族后再查询')
        return
      }
      ...
      // 使用动态API地址
      apiService.post(`${this.apiBaseUrl}/query_family`, { tableName: this.tableName.trim() })
        .then(response => {
    ...
    toggleDetails(sha256) {
      // 切换详情显示状态 - 使用Vue3的方式确保响应式
      if (!this.showDetails.hasOwnProperty(sha256)) {
        // 第一次点击,初始化为false
      ...
      // 仅标记当前条目的加载状态,不影响全局
      if (!this.loadingDetails.hasOwnProperty(sha256)) {
        this.loadingDetails = { ...this.loadingDetails, [sha256]: false }
      }
      this.loadingDetails[sha256] = true

      // 使用动态API地址
      apiService.get(`${this.apiBaseUrl}/detail_family/${sha256}`)
        .then(response => {
          // 核心修复:正确解析后端返回的data字段(之前错读成query_result)
    ...
    async downloadFile(e, sha256) {
      ...
      try {
        ...
        const response = await apiService.get(`${this.apiBaseUrl}/download_family/${sha256}`, {
