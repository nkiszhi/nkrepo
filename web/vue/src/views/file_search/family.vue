...
export default {  
  ...
  methods: {  
    searchVirus() {  
      ...
      axios.post('http://10.134.2.27:5005/query_family', { tableName: this.tableName })  
        .then(response => {  
    ...
    toggleDetails(sha256) {  
     ...
     axios.get(`http://10.134.2.27:5005/detail_family/${sha256}`) 
         .then(response => {  
    ...
    downloadFile(sha256) {  
      // 构造下载链接  
      const downloadUrl = `http://10.134.2.27:5005/download_family/${sha256}`;  
