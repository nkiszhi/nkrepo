  ...
  <main>
    ...
    <div>
      ...
      <div class="drop" @drop="handleDrop" @dragover="handleDragover">
        把待检文件拖到这里或
        <el-button :loading="loading" style="margin-left:0%;font-size: 20px;" size="small" type="primary" @click="handleUpload">
          选择待检文件
        </el-button>
      </div>
      <div v-if="uploadResult">
        <table class="file-info-table">
          <tbody>
            ...
            <!-- 修复：使用过滤后的计算属性，移除v-if -->
            <tr v-for="(value, key) in filteredQueryResult" :key="key">
              <td>{{ key.replace('_', ' ') }}：</td>
              <td>{{ value }}</td>
            </tr>
          </tbody>
...
export default {
  ...
  // 新增：计算属性过滤query_result中的无效值（nan/NaN）
  computed: {
    filteredQueryResult() {
      // 避免uploadResult未加载时的报错
      if (!this.uploadResult || !this.uploadResult.query_result) {
        return {}
      }
      // 过滤逻辑：只保留非nan/NaN的键值对
      const validResult = {}
      Object.entries(this.uploadResult.query_result).forEach(([key, value]) => {
  ...
  methods: {
    ...
    fetchDetailAPI() {
      // 修复：提前定义辅助函数，避免调用时未定义
      this.checkAndUpdateUIs = () => {
        if (!this.isElLoadings) return
        this.$forceUpdate()
        this.isElLoadings = false
      }

      this.checkAndUpdateUI = () => {
        if (!this.isElLoading) return
        this.$forceUpdate()
